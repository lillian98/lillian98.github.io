...
function showCoupon() {
    var _status = $('.it-coupon').find('.status').html();
    if (_status == 1) {
        $('.it-coupon').show();
    }
}
...
function marketRendered() {
    ...
    var timer = function (_selector) {
        var $sel = $(_selector),
            _hour = $sel.find('.hh'),
            _minute = $sel.find('.mm'),
            _second = $sel.find('.ss'),
            _timing, _et = $sel.find('.dataTime').attr('date');
        var t = hh = _hh = 0;

        function serverTime() {
            $.ajax({
                url: 'http://acen.jd.com/time/getLeavTime.html',
                dataType: 'jsonp',
                success: function (data) {
                    window._timing = data - new Date().getTime();
                    setTime();
                }
            })
        }
        ...
        function setTime() {
            ...
            nt = new Date().getTime() + window._timing;
            hh = new Date().getHours();
            _hh = new Date(endTime).getHours();

            window.t = new Date(endTime).getTime() - nt;
            if (window.t >= 0) {
                setHtml();
            }
        ...
        function setHtml() {
            var h = Math.floor(window.t / 1000 / 60 / 60 % 24),
                m = Math.floor(window.t / 1000 / 60 % 60),
                s = Math.floor(window.t / 1000 % 60);

            _hour.html(fillZero(h));
            _minute.html(fillZero(m));
            _second.html(fillZero(s));
        }

        function nextDay() {
            var nextDay, today, y, m, d;
            today = new Date();
