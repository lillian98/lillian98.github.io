...
!function (window) {
    ...
    App.Opa = App.Opa || Opa, Opa.prototype = {init: function () {
        ...
        for (var i = 0, len = pans.length; len > i; i++) {
            ...
            /* @fixme：看这里有什么不一样 */
            item.style.position = "absolute", item.style.left = "0", item.style.top = "0", item.style.width = "100%", item.style.height = "100%", 0 !== i || (util.addClass(item, "current"), util.addClass(item, "start-animate")), item.setAttribute("data-index", i)
        }
    }, resize: function () {
        ...
        function exchange(ev) {
            function moveHandler(ev) {
                posObj = ev.touches ? ev.touches[0] : ev, move.y = posObj.clientY || posObj.pageY, disy = move.y - start.y, Math.abs(disy) > 20 && (disy > 0 ? target.getAttribute("data-index") > 0 ? (next = _this.getNext.call(_this, index, -1), util.addClass(next, "current"), next.style[pre + "Transform"] = "scale(" + disy / vh + ")", next.style.transform = "scale(" + disy / vh + ")") : next = null : target.getAttribute("data-index") < len - 1 ? (next = _this.getNext.call(_this, index, 1), next.style[pre + "Transform"] = "scale(" + -disy / vh + ")", next.style.transform = "scale(" + -disy / vh + ")") : next = null, next && util.addClass(next, "current")), ev.preventDefault()
            }

            function endHandler(ev) {
                next ? (posObj = ev.changedTouches ? ev.changedTouches[0] : ev, move.y = posObj.clientY || posObj.pageY, disy = move.y - start.y, next = disy > 0 ? _this.getNext.call(_this, index, -1) : _this.getNext.call(_this, index, 1), util.addClass(next, "has-transition"), Math.abs(disy) > _this.get("diff") ? (next.style[pre + "Transform"] = "scale(1)", next.style.transform = "scale(1)", setTimeout(function () {
                    util.removeClass(page.querySelector(".sublayer"), "sublayer"), util.removeClass(next, "has-transition"), _this.preLoad(next.getAttribute("data-index")), _this.set("prevent", !1), _this.get("transEnd") && _this.get("transEnd").call(_this)
                }, 300)) : (next.style[pre + "Transform"] = "scale(0)", next.style.transform = "scale(0)", setTimeout(function () {
                    _this.set("prevent", !1), util.removeClass(target, "sublayer"), util.addClass(target, "current"), util.removeClass(next, "current"), util.removeClass(next, "has-transition")
                }, 300))) : (_this.set("prevent", !1), util.addClass(target, "current"), target.getAttribute("data-index") == len - 1 && _this.get("pb") && (_this.get("pbTimer") && (clearTimeout(_this.get("pbTimer")), _this.pbTimer = null), _this.get("pb").style.visibility = "visible", _this.get("pb").style.zIndex = 100, _this.get("pb").style.opacity = 1)), page.removeEventListener(EVENTS.touchmove, moveHandler, !1), page.removeEventListener(EVENTS.touchup, endHandler, !1)
            }
            ...
            start.y = posObj.clientY || posObj.pageY, util.removeClass(target, "current"), util.addClass(target, "sublayer"), page.addEventListener(EVENTS.touchmove, moveHandler, !1), page.addEventListener(EVENTS.touchup, endHandler, !1)
        }
