    ...
    App.Opa = App.Opa || Opa, Opa.prototype = {init: function () {
        ...
        function exchange(ev) {
            function moveHandler(ev) {
                posObj = ev.touches ? ev.touches[0] : ev, move.y = posObj.clientY || posObj.pageY, disy = move.y - start.y, Math.abs(disy) > 20 && (disy > 0 ? target.getAttribute("data-index") > 0 ? (next = _this.getNext.call(_this, index, -1), next.style[pre + "Transform"] = "translate3d(0, -" + (vh - disy) + "px, 0)", next.style.transform = "translate3d(0, -" + (vh - disy) + "px, 0)") : next = null : target.getAttribute("data-index") < len - 1 ? (next = _this.getNext.call(_this, index, 1), next.style[pre + "Transform"] = "translate3d(0, " + (vh + disy) + "px, 0)", next.style.transform = "translate3d(0, " + (vh + disy) + "px, 0)") : next = null, next && util.addClass(next, "current")), ev.preventDefault()
            }

            function endHandler(ev) {
                next ? (posObj = ev.changedTouches ? ev.changedTouches[0] : ev, move.y = posObj.clientY || posObj.pageY, disy = move.y - start.y, next = disy > 0 ? _this.getNext.call(_this, index, -1) : _this.getNext.call(_this, index, 1), util.addClass(next, "has-transition"), Math.abs(disy) > _this.get("diff") ? (next.style[pre + "Transform"] = "translate3d(0, 0, 0)", next.style.transform = "translate3d(0, 0, 0)", setTimeout(function () {
                ...
                }, 400)) : (next.style[pre + "Transform"] = "translate3d(0, " + (disy > 0 ? -1 : 1) * vh + "px, 0)", next.style.transform = "translate3d(0, " + (disy > 0 ? -1 : 1) * vh + "px, 0)", util.removeClass(target, "sublayer"), util.addClass(target, "current"), util.removeClass(next, "current"), setTimeout(function () {
                    _this.set("prevent", !1), util.removeClass(next, "has-transition")
                }, 400))) : (util.addClass(target, "current"), util.addClass(target, "has-transition"), target.style[pre + "Transform"] = "translate3d(0, 0, 0)", target.style.transform = "translate3d(0, 0, 0)", target.getAttribute("data-index") == len - 1 && _this.get("pb") && (_this.get("pbTimer") && (clearTimeout(_this.get("pbTimer")), _this.pbTimer = null), _this.get("pb").style.visibility = "visible", _this.get("pb").style.zIndex = 100, _this.get("pb").style.opacity = 1), setTimeout(function () {
