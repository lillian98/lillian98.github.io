...
!function (window) {
    ...
    var curPageNo = (curHash!='')?parseInt(curHash.split('#')[1]): 0,pageWrap = document.querySelector('.mc-wrap-page'),pageIcons = pageWrap.querySelectorAll('.icon'), hrefBack = document.querySelector('#hrefBack');
    var curHrefArray = window.location.href.split('/');
    var curHrefName = curHrefArray[curHrefArray.length-1];
    var curMainNo = parseInt(curHrefName.split('.html')[0])-1;
    App.Opa = App.Opa || Opa, Opa.prototype = {init: function () {
        var len = 0;
        App.Utils.addClass(pageIcons[curPageNo],'page-current'),App.Utils.addClass(hrefBack, "href-back-init");
        this.set("pageNode", document.getElementById(this.get("page")) || document.body), this.set("panNodes", this.get("pageNode").querySelectorAll("." + this.get("pannel"))), len = this.get("panNodes").length, this.set("vw", body.clientWidth > 750 ? 750 : body.clientWidth), this.set("vh", this.get("size").height || body.clientHeight), this.set("pb", document.querySelector("#J_PageBack")), this.set("current", curPageNo), this.set("circle", !1), this.set("diff", 100), this.set("prefix", this.getPrefix(browser)), this.set("prevent", !1), this._parseNode(), !len || (this.preLoad(0), this.bindEvent(), this.pbEvent())
        ...
        function exchange(ev) {
            function moveHandler(ev) {
                ...
                next.style.opacity = disy / vh) : (console.log('1111'),next = null)) : target.getAttribute("data-index") < len - 1 ? (next = _this.getNext.call(_this, index, 1),
                ...
                target.style.opacity = 1 + disy / vh) : (console.log('2222'),next = null)), ev.preventDefault()
            }
            ...
            function endHandler(ev) {
                ...
                next ? (posObj = ev.changedTouches ? ev.changedTouches[0] : ev, 
                    ...
                    util.addClass(next, "has-transition"), util.addClass(target, "has-transition"), util.addClass(hrefBack, "href-back-hide"),
                    Math.abs(disy) > _this.get("diff") ? disy > 0 ? (next.style[pre + "Transform"] = "translate3d(0, 0, 0)", 
                    next.style.transform = "translate3d(0, 0, 0)", next.style.opacity = 1, util.addClass(pageIcons[next.getAttribute('data-index')],'page-current'),util.removeClass(pageIcons[index], "page-current"), setTimeout(function () {
                        util.removeClass(hrefBack, "href-back-hide"),util.removeClass(target, "has-transition"), util.removeClass(next, "has-transition"), util.removeClass(target, "sublayer"), _this.preLoad(next.getAttribute("data-index")), _this.set("prevent", !1), _this.get("transEnd") && _this.get("transEnd").call(_this)
                    ...
                    util.removeClass(target, "current"), util.addClass(next, "current"), util.addClass(pageIcons[next.getAttribute('data-index')],'page-current'),
                    util.removeClass(next, "sublayer"), util.addClass(pageIcons[next.getAttribute('data-index')],'page-current'),util.removeClass(pageIcons[index], "page-current"),util.removeClass(hrefBack, "href-back-hide"),
