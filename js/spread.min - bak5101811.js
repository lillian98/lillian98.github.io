...
if(curDom == null || curDom == 'undefined'){
    /*console.log('2222222222222 move null')
 curDom = this.querySelector('.secene-sec-current'),
                    evtDom = this.querySelector('.current .mc-wrap-detail'),
                    ...
                    nowSecIndex = parseInt(curDom.getAttribute('data-sec-index')),
                        ...
                        console.log('2222222222 move not null',this)
                       curDom = this.querySelector('.secene-sec-current'),
                    evtDom = this,
                        nowSecDom = evtDom.querySelectorAll('.secene-sec'),
                    nowSecIndex = parseInt(curDom.getAttribute('data-sec-index')),
                        nowSecLength = nowSecDom.length;
                        console.log('222222222 move',curDom,nowSecDom);
if(Math.abs(disy)>20){
        ...
        if(nowSecIndex < nowSecLength -1){
                ...
                if(document.querySelector("#J_opaPage").className.indexOf('float-show')>-1 && document.querySelector("#J_opaPage").className.indexOf('float-show-') <0){
                    ...
                    if(curDom == null || curDom == 'undefined'){
                       ...
                       posObj = ev.changedTouches ? ev.changedTouches[0] : ev, move.y = posObj.clientY || posObj.pageY, disy = move.y - start.y;
                        ...
                        console.log('3333333333333333 end',curDom,next);                
                        if(next){
                        ...
                        util.removeClass(curDom, "secene-sec-current"), util.removeClass(curDom, "has-transition"), util.addClass(next, "secene-sec-current"), util.removeClass(next, "has-transition"), _this.preLoad(curDom.getAttribute("data-sec-index")), _this.set("prevent", !1), _this.get("transEnd") && _this.get("transEnd").call(_this)}, 600)
                        }
                        else{
                        ...
                        _this.set("prevent", !1), util.removeClass(curDom, "has-transition"), util.removeClass(next, "has-transition")}, 600)
                        }
                    }
                    else{
                        _this.set("prevent", !1);        
                        curDom.getAttribute("data-index") == nowSecLength - 1 && _this.get("pb") && (_this.get("pbTimer") && (clearTimeout(_this.get("pbTimer")), _this.pbTimer = null), _this.get("pb").style.visibility = "visible", _this.get("pb").style.zIndex = 100, _this.get("pb").style.opacity = 1)                                           
                    }                    
            ...
            if(document.querySelector("#J_opaPage").className.indexOf('float-show')>-1 && document.querySelector("#J_opaPage").className.indexOf('float-show-') <0){
                /*if (_this.get("prevent"))return!1;
                    ...
                    start.y = posObj.clientY || posObj.pageY,evtDom.addEventListener(EVENTS.touchmove, moveHandler, !1), evtDom.addEventListener(EVENTS.touchup, endHandler, !1)
                    return false;
                }
                else{
                    if (_this.get("prevent"))return!1;
