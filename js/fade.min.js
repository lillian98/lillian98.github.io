...
!function (window) {
    ...
    App.Opa = App.Opa || Opa, Opa.prototype = {init: function () {
        ...
        for (var i = 0, len = pans.length; len > i; i++) {
            ...
            item.style.position = "absolute", item.style.left = "0", item.style.top = "0", item.style.width = "100%", item.style.height = "100%", 0 !== i ? item.style.opacity = 0 : (util.addClass(item, "current"), util.addClass(item, "start-animate")), item.setAttribute("data-index", i)
        }
    }, resize: function () {
        ...
        function exchange(ev) {
            function moveHandler(ev) {
                posObj = ev.touches ? ev.touches[0] : ev, move.y = posObj.clientY || posObj.pageY, disy = move.y - start.y, Math.abs(disy) > 20 && (disy > 0 ? (util.removeClass(target, "current"), util.addClass(target, "sublayer"), target.getAttribute("data-index") > 0 ? (next = _this.getNext.call(_this, index, -1), util.addClass(next, "current"), next.style[pre + "Transform"] = "translate3d(0, -" + (vh - disy) + "px, 0)", next.style.transform = "translate3d(0, -" + (vh - disy) + "px, 0)", next.style.opacity = disy / vh) : next = null) : target.getAttribute("data-index") < len - 1 ? (next = _this.getNext.call(_this, index, 1), util.addClass(next, "sublayer"), next.style.opacity = 1, target.style[pre + "Transform"] = "translate3d(0, " + disy + "px, 0)", target.style.transform = "translate3d(0, " + disy + "px, 0)", target.style.opacity = 1 + disy / vh) : next = null), ev.preventDefault()
            }

            function endHandler(ev) {
                next ? (posObj = ev.changedTouches ? ev.changedTouches[0] : ev, move.y = posObj.clientY || posObj.pageY, disy = move.y - start.y, next = disy > 0 ? _this.getNext.call(_this, index, -1) : _this.getNext.call(_this, index, 1), util.addClass(next, "has-transition"), util.addClass(target, "has-transition"), Math.abs(disy) > _this.get("diff") ? disy > 0 ? (next.style[pre + "Transform"] = "translate3d(0, 0, 0)", next.style.transform = "translate3d(0, 0, 0)", next.style.opacity = 1, setTimeout(function () {
                ...
                }, 400)) : (target.style[pre + "Transform"] = "translate3d(0, -" + vh + "px, 0)", target.style.transform = "translate3d(0, -" + vh + "px, 0)", target.style.opacity = 0, setTimeout(function () {
                    util.removeClass(target, "has-transition"), util.removeClass(next, "has-transition"), util.removeClass(target, "current"), util.addClass(next, "current"), util.removeClass(next, "sublayer"), _this.preLoad(next.getAttribute("data-index")), _this.set("prevent", !1), _this.get("transEnd") && _this.get("transEnd").call(_this)
