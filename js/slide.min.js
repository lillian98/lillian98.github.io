...
!function (window) {
    ...
    var _default = {page: "J_opaPage", dragger: "J_opaScroll", pannel: "J_opaPannel", transEnd: function () {
    ...
    }, EVENTS = {touchdown: "ontouchstart"in window ? "touchstart" : "mousedown", touchmove: "ontouchmove"in window ? "touchmove" : "mousemove", touchup: "ontouchend"in window ? "touchend" : "mouseup"};
    Opa.prototype = {init: function () {
        this.set("pageNode", document.querySelector("#" + this.get("page"))), this.set("pannels", this.get("pageNode").querySelectorAll("." + this.get("pannel"))), this.set("count", this.get("pannels").length), this.set("offset", {left: 0, top: 0}), this.get("size") ? (this.set("vw", this.get("size").width || (body.clientWidth > 750 ? 750 : body.clientWidth)), this.set("vh", this.get("size").height || body.clientHeight)) : (this.set("vw", body.clientWidth > 750 ? 750 : body.clientWidth), this.set("vh", body.clientHeight)), this.set("prefix", css3Surport().prefix), this.get("prefix") ? this.set("transform", this.get("prefix") + "Transform") : this.set("transform", "transform"), this.set("guideTop", document.querySelector("#J_GuideTop")), this.set("prevent", !1), this.parsePannel(), this.preload(0), this.bindEvent()
        ...
        var _this = this, page = _this.get("pageNode"), dragger = _this.get("dragger"), count = (_this.get("pannels"), _this.get("count")), vw = _this.get("vw"), ofs = {}, start = {x: 0, y: 0}, end = {x: 0, y: 0}, dis = 0, touchStart = function (ev) {
            ...
            var touchMove = function (ev) {
                ev.preventDefault(), evo = ev.touches ? ev.touches[0] : ev, end = {x: evo.clientX, y: evo.clientY}, dis = end.x - start.x, Math.abs(dis) > 20 && (dragger.style[_this.get("transform")] = "translate3d(" + 100 * -(ofs.left / count - dis / (vw * count)) + "%, 0, 0)")
            }, touchEnd = function () {
                ...
                if (Utils.addClass(dragger, "has-transition"), next)if (Math.abs(dis) > 50) {
                    ...
                    dragger.style[_this.get("transform")] = "translate3d(" + -(nextTop / count * 100) + "%, 0, 0)", Utils.removeClass(current, "current"), Utils.addClass(next, "current"), _this.set("offset", {left: nextTop, top: 0}), _this.preload(+next.getAttribute("data-index")), setTimeout(function () {
